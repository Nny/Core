    ...
    switch(zoneid)
    {
		...
		case 4384:                                          // SA
            ...
            else
            {
				...
				FillInitialWorldState(data,count, 0xf09, 0x4);// 7  3849 Gate of Temple
				FillInitialWorldState(data,count, 0xe36, 0x4);// 8  3638 Gate of Yellow Moon
				FillInitialWorldState(data,count, 0xe27, 0x4);// 9  3623 Gate of Green Emerald
				FillInitialWorldState(data,count, 0xe24, 0x4);// 10 3620 Gate of Blue Sapphire
				FillInitialWorldState(data,count, 0xe21, 0x4);// 11 3617 Gate of Red Sun
				FillInitialWorldState(data,count, 0xe1e, 0x4);// 12 3614 Gate of Purple Ametyst
				FillInitialWorldState(data,count, 0xdf3, 0x0);// 13 3571 bonus timer (1 - on, 0 - off)
				FillInitialWorldState(data,count, 0xded, 0x0);// 14 3565 Horde Attacker
				FillInitialWorldState(data,count, 0xdec, 0x1);// 15 3564 Alliance Attacker
                // End Round (timer), better explain this by example, eg. ends in 19:59 -> A:BC
				FillInitialWorldState(data,count, 0xde9, 0x9);// 16 3561 C
				FillInitialWorldState(data,count, 0xde8, 0x5);// 17 3560 B
				FillInitialWorldState(data,count, 0xde7, 0x19);// 18 3559 A
				FillInitialWorldState(data,count, 0xe35, 0x1);// 19 3637 East g - Horde control
				FillInitialWorldState(data,count, 0xe34, 0x1);// 20 3636 West g - Horde control
				FillInitialWorldState(data,count, 0xe33, 0x1);// 21 3635 South g - Horde control
				FillInitialWorldState(data,count, 0xe32, 0x0);// 22 3634 East g - Alliance control
				FillInitialWorldState(data,count, 0xe31, 0x0);// 23 3633 West g - Alliance control
				FillInitialWorldState(data,count, 0xe30, 0x0);// 24 3632 South g - Alliance control
				FillInitialWorldState(data,count, 0xe2f, 0x1);// 25 3631 Chamber of Ancients - Horde control
				FillInitialWorldState(data,count, 0xe2e, 0x0);// 26 3630 Chamber of Ancients - Alliance control
				FillInitialWorldState(data,count, 0xe2d, 0x0);// 27 3629 Beach1 - Horde control
				FillInitialWorldState(data,count, 0xe2c, 0x0);// 28 3628 Beach2 - Horde control
				FillInitialWorldState(data,count, 0xe2b, 0x1);// 29 3627 Beach1 - Alliance control
				FillInitialWorldState(data,count, 0xe2a, 0x1);// 30 3626 Beach2 - Alliance control
                // and many unks...
            }
            break;
		case 5031:                                          // TP
